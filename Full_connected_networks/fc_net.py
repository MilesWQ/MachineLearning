...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        # L the Lth hidden layer, H number of neurons of Lth layer
        # each weight shape previous_H x H
        previous_layer_H = input_dim
        for L, H in enumerate(hidden_dims):
            key = 'W' + str(L+1)
            self.params[key] = weight_scale * \
                np.random.randn(previous_layer_H, H)
            key = 'b' + str(L+1)
            self.params[key] = np.zeros(H)
            previous_layer_H = H
        # weights for output layer
        key = 'W' + str(self.num_layers)
        self.params[key] = weight_scale * \
            np.random.randn(previous_layer_H, num_classes)
