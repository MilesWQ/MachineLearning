from util import load_data, np, plt, rel_error, eval_numerical_gradient_array, eval_numerical_gradient
from layers import affine_forward, affine_backward, relu_forward, relu_backward
from layer_utils import affine_relu_forward, affine_relu_backward, svm_loss, softmax_loss
from fc_net import TwoLayerNet, FullyConnectedNet
...
"""
# test mini data training accuracy 100% with more layers the loss is more sensitive to weight initializing
cifar_dir = '../cifar-10-batches-py'
X_train, y_train, X_val, y_val, X_test, y_test, X_dev, y_dev = load_data(
    cifar_dir, num_dev=50)

data = {'X_train': X_dev, 'y_train': y_dev, 'X_val': X_val, 'y_val': y_val}

weight_scale = 1e-1
learning_rate = 2e-4
# 3 layer NN weight_scale = 1e-1 learning_rate = 2e-4
#model = FullyConnectedNet([100, 100], weight_scale=weight_scale, dtype=np.float64)
# 5 layer NN weight_scale = 1e-1 learning_rate = 2e-4
model = FullyConnectedNet([100, 100, 100, 100],
                          weight_scale=weight_scale, dtype=np.float64)
solver = Solver(model, data, print_every=10, num_epochs=20, batch_size=25, update_rule='sgd', optim_config={
