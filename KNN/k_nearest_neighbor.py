import numpy as np
"""
copy from cs231n http://cs231n.github.io/
"""
...
class KNearestNeighbor(object):
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            #########################################################################
            # index list with sorted item order, the sort_dists_idx represnet the example indexes in X_train
            sort_dists_idx = np.argsort(dists[i, ])
            ...
            if len(closest_y) != len(unique_label):
                ...
                for label in closest_y:
                    num_label = closest_y.count(label)
                    if num_label > common:
                        common = num_label
                        y_pred[i] = label
            else:
